...
export type ProvisionCloneInput = {
  ...
  // plan — `armGate` decides that, not the wizard.
...
export const provisionClone = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<ProvisionCloneResult> => {
    ...
    // it runs out, until Stripe captures the activation payment. This is the
    // only place a gate is ever created: nothing backfills, so the prime and
    // every clone that already exists are untouched by construction rather
    // than by a flag somebody has to remember to set.
    //
    // Deliberately non-fatal. The repo is already forked and the clone row is
    // already written; failing here would leave a half-provisioned clone
    // behind a gate that is also the thing that failed. A clone that does not
    // arm is a clone with no gate — the fleet's existing behaviour — and the
    // Payment Gates console lists paid clones with no gate for exactly this
    // reason.
    ...
    if (!gate.armed && gate.reason === "write_failed") {
      console.error("[provisionClone] activation gate not armed", {
        cloneId: inserted.id,
        detail: gate.detail,
      });
    }
